...
void Controlador::manejarEventos(bool& jugador_activo, int& estado) {
    ...
    while (SDL_PollEvent(&sdlEvent)) {
        if (sdlEvent.type == SDL_QUIT) {
            ...
            } else if (sdlEvent.key.keysym.sym == SDLK_w) { // Verifica si la tecla presionada es "e"
                estado = 1; // Cambia el valor de "estado" a 1
            } else if (sdlEvent.key.keysym.sym == SDLK_e) { // Verifica si la tecla presionada es "e"
                estado = 2; // Cambia el valor de "estado" a 2
            } else if (sdlEvent.key.keysym.sym == SDLK_r) { // Verifica si la tecla presionada es "e"
                estado = 3; // Cambia el valor de "estado" a 3
            } else if (sdlEvent.key.keysym.sym == SDLK_a) { // Verifica si la tecla presionada es "e"
