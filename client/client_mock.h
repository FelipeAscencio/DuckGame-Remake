...
class EnviadorCliente : public Thread{
    ...
    public:
        EnviadorCliente(ProtocoloCliente& p, Queue<char>& q, int id) : protocol(p), queue_comandos(q), id_cliente(id), vivo(true) {}

        void run() override{
            while (vivo){
                char accion;
                try (queue_comandos.pop(accion)){
                    if (!protocol.enviar(accion)) break;
...
class RecibidorCliente{
    ...
    public:
        explicit RecibidorCliente(ProtocoloCliente& p, Queue<EstadoJuego>& q) : protocol(p), queue_estados(q), vivo(true) {}

        void run() override{
            while (vivo){
                EstadoJuego estado_actual;
                if (!protocol.recibir(estado_actual)) break;
                try {
