...
void Dibujador::dibujar_sprites_fila(SDL2pp::Renderer& renderer, SDL2pp::Texture& spriteSheetPato, 
    ...
    int startX = static_cast<int>(anchoVentana * 0.05);        // 5% de la ventana en X para el primer sprite
    int startY = static_cast<int>(altoVentana * 0.2);        // 20% de la ventana en Y para mantenerlo fijo
    int scaledWidth = static_cast<int>(anchoVentana * 0.08);   // Escala del pato al 8% de la anchura de la ventana
    int scaledHeight = static_cast<int>(altoVentana * 0.08); // Escala del pato al 8% de la altura de la ventana
