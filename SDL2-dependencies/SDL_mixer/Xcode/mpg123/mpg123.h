...
/* Simplified large file handling.
    I used to have a check here that prevents building for a library with conflicting large file
   setup (application that uses 32 bit offsets with library that uses 64 bits). While that was
   perfectly fine in an environment where there is one incarnation of the library, it hurt GNU/Linux
   and Solaris systems with multilib where the distribution fails to provide the correct header
   matching the 32 bit library (where large files need explicit support) or the 64 bit library
   (where there is no distinction).
    ...
    More complication: What about client code defining _LARGEFILE64_SOURCE? It might want direct
   access to the _64 functions, along with the ones without suffix. Well, that's possible now via
   defining MPG123_NO_LARGENAME and MPG123_LARGESUFFIX, respectively, for disabling or enforcing the
   suffix names.
*/

/*
    Now, the renaming of large file aware functions.
    By default, it appends underscore _FILE_OFFSET_BITS (so, mpg123_seek_64 for mpg123_seek), if
   _FILE_OFFSET_BITS is defined. You can force a different suffix via MPG123_LARGESUFFIX (that must
   include the underscore), or you can just disable the whole mess by defining MPG123_NO_LARGENAME.
*/
 ...
 * The parameters of handles have workable defaults, you only have to tune them when you want to
 * tune something;-) Tip: Use a RVA setting...
 *
 * @{
 */
...
/** Opaque structure for the libmpg123 decoder handle.
 *  Most functions take a pointer to a mpg123_handle as first argument and operate on its data in an
...
/** Create a handle with optional choice of decoder (named by a string, see mpg123_decoders() or
 * mpg123_supported_decoders()). and optional retrieval of an error code to feed to
 * mpg123_plain_strerror(). Optional means: Any of or both the parameters may be NULL.
 *
...
enum mpg123_parms {
    MPG123_VERBOSE = 0,   /**< set verbosity value for enabling messages to stderr, >= 0 makes sense
                             (integer) */
    ...
    MPG123_OUTSCALE, /**< the scale for output samples (amplitude - integer or float according to
                        mpg123 output format, normally integer) */
    ...
    MPG123_RESYNC_LIMIT, /**< Try resync on frame parsing for that many bytes or until end of stream
                            (<0 ... integer). This can enlarge the limit for skipping junk on
                            beginning, too (but not reduce it).  */
    MPG123_INDEX_SIZE /**< Set the frame index size (if supported). Values <0 mean that the index is
                         allowed to grow dynamically in these steps (in positive direction, of
                         course) -- Use this when you really want a full index with every individual
                         frame. */
    ,
    MPG123_PREFRAMES /**< Decode/ignore that many frames in advance for layer 3. This is needed to
                        fill bit reservoir after seeking, for example (but also at least one frame
                        in advance is needed to have all "normal" data for layer 3). Give a positive
                        integer value, please.*/
    ,
    MPG123_FEEDPOOL /**< For feeder mode, keep that many buffers in a pool to avoid frequent
                       malloc/free. The pool is allocated on mpg123_open_feed(). If you change this
                       parameter afterwards, you can trigger growth and shrinkage during decoding.
                       The default value could change any time. If you care about this, then set it.
                       (integer) */
    ,
    MPG123_FEEDBUFFER /**< Minimal size of one internal feeder buffer, again, the default value is
                         subject to change. (integer) */
    ,
    MPG123_FREEFORMAT_SIZE /**< Tell the parser a free-format frame size to
                            * avoid read-ahead to get it. A value of -1 (default) means that the
                            * parser will determine it. The parameter value is applied during
                            * decoder setup for a freshly opened stream only.
                            */
};

/** Flag bits for MPG123_FLAGS, use the usual binary or to combine. */
enum mpg123_param_flags {
    MPG123_FORCE_MONO = 0x7 /**<     0111 Force some mono mode: This is a test bitmask for seeing if
    ...
    ,
    MPG123_GAPLESS =
            0x40 /**< 01000000 Enable gapless decoding (default on if libmpg123 has support). */
    ,
    MPG123_NO_RESYNC = 0x80 /**< 10000000 Disable resync stream after error. */
    ,
    MPG123_SEEKBUFFER = 0x100 /**< 000100000000 Enable small buffer on non-seekable streams to allow
                                 some peek-ahead (for better MPEG sync). */
    ,
    MPG123_FUZZY = 0x200 /**< 001000000000 Enable fuzzy seeks (guessing byte offsets or using
                            approximate seek points from Xing TOC) */
    ,
    MPG123_FORCE_FLOAT = 0x400 /**< 010000000000 Force floating point output (32 or 64 bits depends
                                  on mpg123 internal precision). */
    ,
    MPG123_PLAIN_ID3TEXT = 0x800 /**< 100000000000 Do not translate ID3 text data to UTF-8. ID3
                                    strings will contain the raw text data, with the first byte
                                    containing the ID3 encoding code. */
    ,
    MPG123_IGNORE_STREAMLENGTH =
            0x1000 /**< 1000000000000 Ignore any stream length information contained in the stream,
    ...
    ,
    MPG123_IGNORE_INFOFRAME = 0x4000 /**< 100 0000 0000 0000 Do not parse the LAME/Xing info frame,
                                        treat it as normal MPEG data. */
    ,
    MPG123_AUTO_RESAMPLE =
            0x8000 /**< 1000 0000 0000 0000 Allow automatic internal resampling of any kind (default
                      on if supported). Especially when going lowlevel with replacing output buffer,
                      you might want to unset this flag. Setting MPG123_DOWNSAMPLE or
                      MPG123_FORCE_RATE will override this. */
    ,
    ...
    MPG123_FLOAT_FALLBACK =
            0x800000 /**< Consider floating point output encoding only after
                      ...
                      * support efficient playback where floating point output is only configured
                      * for an external resampler, bypassing that resampler when the desired rate
                      * can be produced directly. This is enabled by default to be closer to older
                      * versions of libmpg123 which did not enable float automatically at all. If
                      * disabled, float is considered after the 16 bit default and higher-bit
                      * integer encodings for any rate. */
    ,
    MPG123_NO_FRANKENSTEIN =
            0x1000000 /**< Disable support for Frankenstein streams
                       * (different MPEG streams stiched together). Do not accept serious change of
                       * MPEG header inside a single stream. With this flag, the audio output format
                       * cannot change during decoding unless you open a new stream. This also stops
                       * decoding after an announced end of stream (Info header contained a number
                       * of frames and this number has been reached). This makes your MP3 files
                       * behave more like ordinary media files with defined structure, rather than
                       * stream dumps with some sugar. */
};
 ...
 */
MPG123_EXPORT int mpg123_param(mpg123_handle* mh, enum mpg123_parms type, long value,
                               double fvalue);
#endif
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getparam(mpg123_handle* mh, enum mpg123_parms type, long* value,
                                  double* fvalue);
#endif
...
enum mpg123_feature_set {
    MPG123_FEATURE_ABI_UTF8OPEN = 0 /**< mpg123 expects path names to be given in UTF-8 encoding
                                       instead of plain native. */
    ,
...
enum mpg123_errors {
    ...
    MPG123_NEW_FORMAT =
            -11, /**< Message: Output format will be different on next call. Note that some
                    libmpg123 versions between 1.4.3 and 1.8.0 insist on you calling
                    mpg123_getformat() after getting this message code. Newer verisons behave like
                    advertised: You have the chance to call mpg123_getformat(), but you can also
                    just continue decoding and get your data. */
    MPG123_NEED_MORE = -10,   /**< Message: For feed reader: "Feed me more!" (call mpg123_feed() or
    ...
    ,
    MPG123_LFS_OVERFLOW /**< Offset value overflow during translation of large file API calls --
 ...
 *  When a function operating on an mpg123 handle returns MPG123_ERR, you should check for the
 * actual reason via char *errmsg = mpg123_strerror(mh) This function will catch mh == NULL and
 * return the message for MPG123_BAD_HANDLE. \param mh handle \return error message
 */
 ...
 * Perhaps the most prominent feature of mpg123: You have several (optimized) decoders to choose
 ...
 * read ahead ...
 *
 * The mpg123 library decides what output format to use when encountering the first frame in a
 * stream, or actually any frame that is still valid but differs from the frames before in the
 * prompted output format. At such a deciding point, an internal table of allowed encodings,
 * sampling rates and channel setups is consulted. According to this table, an output format is
 * chosen and the decoding engine set up accordingly (including optimized routines for different
 * output formats). This might seem unusual but it just follows from the non-existence of "MPEG
 * audio files" with defined overall properties. There are streams, streams are concatenations of
 * (semi) independent frames. We store streams on disk and call them "MPEG audio files", but that
 * does not change their nature as the decoder is concerned (the LAME/Xing header for gapless
 * decoding makes things interesting again).
 *
 * To get to the point: What you do with mpg123_format() and friends is to fill the internal table
 * of allowed formats before it is used. That includes removing support for some formats or adding
 * your forced sample rate (see MPG123_FORCE_RATE) that will be used with the crude internal
 * resampler. Also keep in mind that the sample encoding is just a question of choice -- the MPEG
 * frames do only indicate their native sampling rate and channel count. If you want to decode to
 * integer or float samples, 8 or 16 bit ... that is your decision. In a "clean" world, libmpg123
 * would always decode to 32 bit float and let you handle any sample conversion. But there are
 * optimized routines that work faster by directly decoding to the desired encoding / accuracy. We
 * prefer efficiency over conceptual tidyness.
 *
 * People often start out thinking that mpg123_format() should change the actual decoding format on
 * the fly. That is wrong. It only has effect on the next natural change of output format, when
 * libmpg123 will consult its format table again. To make life easier, you might want to call
 * mpg123_format_none() before any thing else and then just allow one desired encoding and a limited
 * set of sample rates / channel choices that you actually intend to deal with. You can force
 * libmpg123 to decode everything to 44100 KHz, stereo, 16 bit integer ... it will duplicate mono
 * channels and even do resampling if needed (unless that feature is disabled in the build, same
 * with some encodings). But I have to stress that the resampling of libmpg123 is very crude and
 * doesn't even contain any kind of "proper" interpolation.
 *
 * In any case, watch out for MPG123_NEW_FORMAT as return message from decoding routines and call
 * mpg123_getformat() to get the currently active output format.
 *
 * @{
 */
 ...
 * (MPG123_ENC_SIGNED_16, not MPG123_SIGNED). \param list Store a pointer to the encodings array
 * there. \param number Store the number of encodings there. */
 ...
 *  \param encodings A combination of accepted encodings for rate and channels, p.ex
 * MPG123_ENC_SIGNED16 | MPG123_ENC_ULAW_8 (or 0 for no support). Please note that some encodings
 * may not be supported in the library build and thus will be ignored here. \return MPG123_OK on
 * success, MPG123_ERR if there was an error. */
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getformat2(mpg123_handle* mh, long* rate, int* channels, int* encoding,
                                    int clear_flag);
 ...
 * Decoding/seek functions may also return message codes MPG123_DONE, MPG123_NEW_FORMAT and
 ...
 */
MPG123_EXPORT int mpg123_open_fixed(mpg123_handle* mh, const char* path, int channels,
                                    int encoding);
 ...
 *  This works together with mpg123_decode(); you are responsible for reading and feeding the input
 * bitstream. Also, you are expected to handle ICY metadata extraction yourself. This input method
 * does not handle MPG123_ICY_INTERVAL. It does parse ID3 frames, though. \param mh handle \return
 ...
 *  Think of this function being the union of mpg123_read() and mpg123_feed() (which it actually is,
 * sort of;-). You can actually always decide if you want those specialized functions in separate
 ...
 */
MPG123_EXPORT int mpg123_decode(mpg123_handle* mh, const unsigned char* inmemory, size_t inmemsize,
 ...
 *  \return MPG123_OK or error/message code
 */
MPG123_EXPORT int mpg123_decode_frame(mpg123_handle* mh, off_t* num, unsigned char** audio,
                                      size_t* bytes);
 ...
 *  \return MPG123_OK or error/message code
 */
MPG123_EXPORT int mpg123_framebyframe_decode(mpg123_handle* mh, off_t* num, unsigned char** audio,
                                             size_t* bytes);
 ...
 * Together with the raw header, you can reconstruct the whole raw MPEG stream without junk and meta
 * data, or play games by actually modifying the frame body data before decoding this frame
 * (mpg123_framebyframe_decode()). A more sane use would be to use this for CRC checking (see
 * mpg123_info() and MPG123_CRC), the first two bytes of the body make up the CRC16 checksum, if
 * present. You can provide NULL for a parameter pointer when you are not interested in the value.
 *
 * \param mh handle
 ...
 * \return The resulting offset >= 0 or error/message code
 */
MPG123_EXPORT off_t mpg123_feedseek(mpg123_handle* mh, off_t sampleoff, int whence,
                                    off_t* input_offset);
 ...
 *  This assumes that the samples per frame do not change in the file/stream, which is a good
 * assumption for any sane file/stream only. \return frame offset >= 0 or error/message code */
 ...
 */
MPG123_EXPORT int mpg123_position(mpg123_handle* mh, off_t frame_offset, off_t buffered_bytes,
...
struct mpg123_frameinfo {
    ...
    enum mpg123_flags
            flags; /**< MPEG Audio flag bits. Just now I realize that it should be declared as int,
                      not enum. It's a bitwise combination of the enum values. */
...
enum mpg123_state {
    MPG123_ACCURATE = 1 /**< Query if positons are currently accurate (integer value, 0 if false, 1
                           if true). */
    ,
    MPG123_BUFFERFILL /**< Get fill of internal (feed) input buffer as integer byte count returned
                         as long and as double. An error is returned on integer overflow while
                         converting to (signed) long, but the returned floating point value shold
                         still be fine. */
    ,
    MPG123_FRANKENSTEIN /**< Stream consists of carelessly stitched together files. Seeking may
                           yield unexpected results (also with MPG123_ACCURATE, it may be confused).
                         */
    ,
    MPG123_FRESH_DECODER /**< Decoder structure has been updated, possibly indicating changed stream
                            (integer value, 0 if false, 1 if true). Flag is cleared after retrieval.
                          */
    ,
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getstate(mpg123_handle* mh, enum mpg123_state key, long* val,
                                  double* fval);
#endif
 ...
 *  \return 0 on error, 1 on success
 */
MPG123_EXPORT int mpg123_add_substring(mpg123_string* sb, const char* stuff, size_t from,
                                       size_t count);
 ...
 *  \return 0 on error, 1 on success
 */
MPG123_EXPORT int mpg123_set_substring(mpg123_string* sb, const char* stuff, size_t from,
                                       size_t count);
...
enum mpg123_text_encoding {
    ...
    ,
    mpg123_text_latin1 = 2 /**< ISO-8859-1. Note that sometimes latin1 in ID3 is abused for totally
                              different encodings. */
    ,
    mpg123_text_icy = 3 /**< ICY metadata encoding, usually CP-1252 but we take it as UTF-8 if it
                           qualifies as such. */
    ,
    mpg123_text_cp1252 = 4 /**< Really CP-1252 without any guessing. */
    ,
    mpg123_text_utf16 =
            5 /**< Some UTF-16 encoding. The last of a set of leading BOMs (byte order mark) rules.
               *   When there is no BOM, big endian ordering is used. Note that UCS-2 qualifies as
               * UTF-8 when you don't mess with the reserved code points. If you want to decode
               * little endian data without BOM you need to prepend 0xff 0xfe yourself. */
    ,
    mpg123_text_utf16bom = 6 /**< Just an alias for UTF-16, ID3v2 has this as distinct code. */
    ,
    mpg123_text_utf16be =
            7 /**< Another alias for UTF16 from ID3v2. Note, that, because of the mess that is
               * reality, BOMs are used if encountered. There really is not much distinction between
               * the UTF16 types for mpg123 One exception: Since this is seen in ID3v2 tags, leading
               * null bytes are skipped for all other UTF16 types (we expect a BOM before real data
               * there), not so for utf16be!*/
    ,
 ...
 *  MPG123_ENUM_API is defined.
 *
 *  A prominent error can be that you provided an unknown encoding value, or this build of libmpg123
 * lacks support for certain encodings (ID3 or ICY stuff missing). Also, you might want to take a
 * bit of care with preparing the data; for example, strip leading zeroes (I have seen that). \param
 * sb  target string \param enc mpg123 text encoding value \param source source buffer with plain
 * unsigned bytes (you might need to cast from signed char) \param source_size number of bytes in
 * the source buffer \return 0 on error, 1 on success (on error, mpg123_free_string is called on sb)
 */
MPG123_EXPORT int mpg123_store_utf8(mpg123_string* sb, enum mpg123_text_encoding enc,
 ...
 * lacks support for certain encodings (ID3 or ICY stuff missing). Also, you might want to take a
 * bit of care with preparing the data; for example, strip leading zeroes (I have seen that). \param
 * sb  target string \param enc mpg123 text encoding value (enum mpg123_text_encoding) \param source
 * source buffer with plain unsigned bytes (you might need to cast from signed char) \param
 * source_size number of bytes in the source buffer \return 0 on error, 1 on success (on error,
 * mpg123_free_string is called on sb)
 */
MPG123_EXPORT int mpg123_store_utf8_2(mpg123_string* sb, int enc, const unsigned char* source,
                                      size_t source_size);
...
typedef struct {
    ...
    char id[4];   /**< The ID3v2 text field id, like TALB, TPE2, ... (4 characters, no string
                     termination). */
...
typedef struct {
    ...
    mpg123_string* genre;   /**< Genre String (pointer into text_list). The genre string(s) may very
                               well need postprocessing, esp. for ID3v2.3. */
    ...
    /* Encountered ID3v2 fields are appended to these lists.
       There can be multiple occurences, the pointers above always point to the last encountered
       data. */
...
/** Point v1 and v2 to existing data structures wich may change on any next read/decode function
 * call. v1 and/or v2 can be set to NULL when there is no corresponding data. \return MPG123_OK on
 * success
 */
 ...
 *          values are set.
 */
MPG123_EXPORT int mpg123_id3_raw(mpg123_handle* mh, unsigned char** v1, size_t* v1_size,
 ...
 *  Note that this is very similar to mpg123_store_utf8(&sb, mpg123_text_icy, icy_text,
 * strlen(icy_text+1)) . \param icy_text The input data in ICY encoding \return pointer to newly
 * allocated buffer with UTF-8 data (You free() it!) */
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getpar(mpg123_pars* mp, enum mpg123_parms type, long* value,
                                double* fvalue);
#endif
 ...
 * You may want to do tricky stuff with I/O that does not work with mpg123's default file access or
 * you want to make it decode into your own pocket...
 *
 * @{ */

/** Replace default internal buffer with user-supplied buffer.
 * Instead of working on it's own private buffer, mpg123 will directly use the one you provide for
 * storing decoded audio. Note that the required buffer size could be bigger than expected from
 * output encoding if libmpg123 has to convert from primary decoder output (p.ex. 32 bit storage for
 * 24 bit output).
 *
