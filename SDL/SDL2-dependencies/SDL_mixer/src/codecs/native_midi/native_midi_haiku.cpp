...
bool compareMIDIEvent(const MIDIEvent& a, const MIDIEvent& b) { return a.time < b.time; }
...
protected:
    ...
    void sort_events() {
        ...
        for (x = fEvs; x && x->next; x = x->next) assert(x->time <= x->next->time);
...
void native_midi_stop(void) {
    ...
    while (currentSong->store->IsPlaying()) usleep(1000);
    currentSong = NULL;
}
