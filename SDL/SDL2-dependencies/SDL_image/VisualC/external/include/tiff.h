 ...
 *--:
 * Note2: TIFF_IFD8 data type is used in tiffFields[]-tag definition in order to distinguish the
 write-handling of those tags between ClassicTIFF and BigTiff: For ClassicTIFF libtiff writes a
 32-bit value and the TIFF_IFD type-id into the file For BigTIFF     libtiff writes a 64-bit value
 and the TIFF_IFD8 type-id into the file
 */
typedef enum {
...
#define TIFFTAG_IRASB_TRANSORMATION_MATRIX \
    33920 /* Originally part of Intergraph's GeoTIFF tags, but likely understood by IrasB only. */
...
#define TIFFTAG_DEFLATE_SUBCODEC                                                                   \
    65570 /* ZIP codec: to get/set the sub-codec to use. Will default to libdeflate when available \
           */
...
#define EXIFTAG_SENSITIVITYTYPE                                                               \
    34864 /* The SensitivityType tag indicates which one of the parameters of ISO12232 is the \
             PhotographicSensitivity tag. */
#define EXIFTAG_STANDARDOUTPUTSENSITIVITY                                                         \
    34865 /* This tag indicates the standard output sensitivity value of a camera or input device \
...
#define EXIFTAG_ACCELERATION                                                                      \
    37892 /* Acceleration (a scalar regardless of direction) as the ambient situation at the shot \
             in units of mGal (10-5 m/s^2) */
#define EXIFTAG_CAMERAELEVATIONANGLE                                                               \
    37893 /* Elevation/depression. angle of the orientation of the camera(imaging optical axis) as \
             the ambient situation at the shot in degree from -180deg to +180deg. */
...
#define EXIFTAG_LENSSPECIFICATION                                                                \
    42034 /* minimum focal length (in mm), maximum focal length (in mm), minimum F number in the \
             minimum focal length, and minimum F number in the maximum focal length, */
